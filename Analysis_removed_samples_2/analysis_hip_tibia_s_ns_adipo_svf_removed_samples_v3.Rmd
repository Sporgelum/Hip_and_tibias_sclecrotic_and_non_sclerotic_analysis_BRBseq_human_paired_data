...
train_predictions <- predict(rf_tuned_prox, final_train_data)
...
# Create heatmap of top important genes
top_genes <- importance_df_gini$Gene[1:50]  # Top 50 most important genes
...
# Create heatmap of top important genes
top_genes <- importance_df_accuracy$Gene[1:50]  # Top 50 most important genes
top_genes <- gsub(top_genes,pattern = "\\.",replacement = "-")
valid_genes <- top_genes[top_genes %in% rownames(selected_data)]
heatmap_data <- selected_data[valid_genes, ]
...
for (gene in top_6_genes) {
  ...
  p <- pdp::partial(
    ...
    train = final_train_data,      # The training data used
