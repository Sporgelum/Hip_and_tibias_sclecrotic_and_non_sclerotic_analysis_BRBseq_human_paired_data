...
```
### ADD cartilage_islands to tibiasm heatmap.
### ADD pellet model of tibias, and adipocyte model of tibias. (MAYBE) (to look for tissue specificities)
...
# quick visu sanity check to see selected genes separate.
library(pheatmap)
annotation_col <-  md_df_hip_extended |>  dplyr::select(Group,sampleID,Gender,oarsi_scoring,oabs_scoring,cysts,fibrosis,blood_vessels,cartilage_islands,trabeculae_thickened,inflammation) |> tibble::column_to_rownames("sampleID")
...
combined_data_hips$celltype <- factor(combined_data_hips$tissue_celltype_lowercase)
...
train_data_clean[,(tail(colnames(train_data_clean),15))]
test_data_clean[,(tail(colnames(test_data_clean),15))]
non_numeric_cols = c("condition","Gender","oarsi_scoring","oabs_scoring","cysts","fibrosis","blood_vessels", "trabeculae_thickened", "cartilage_islands","tidemark_integrity","inflammation")
...
# Add metadata (BMI, Age, Gender)
heatmap_annotation <- annotation_col |> 
  dplyr::left_join(md_df_hip_extended[,c("sampleID","Age","Gender","BMI","oarsi_scoring","oabs_scoring","cysts","fibrosis","blood_vessels", "trabeculae_thickened","cartilage_islands","tidemark_integrity", "inflammation")], by = c("sample_2"="sampleID")) |>
  dplyr::select(-ID, -tissue, -type, -sample_2)
...
# Add metadata (BMI, Age, Gender)
heatmap_annotation <- annotation_col |> 
  dplyr::left_join(md_df_hip_extended[,c("sampleID","Age","Gender","BMI","oarsi_scoring","oabs_scoring","cysts","fibrosis","blood_vessels", "trabeculae_thickened", "tidemark_integrity","cartilage_islands", "inflammation")], by = c("sample_2"="sampleID")) |>
  dplyr::select(-ID, -tissue, -type, -sample_2)
...
hip_samples_without_features_data <- hip_samples_without_features_data |> dplyr::left_join(md_df_hip |> dplyr::select(c("sampleID","health_status","Age","Gender","BMI")), by = "sampleID")
...
# DONE HIPs   
## Maybe use adipocytes and pellet models, if enough samples!
#=======================================
## ADD adipocyte and pellet information on the models in hips and tibias.
#=======================================
