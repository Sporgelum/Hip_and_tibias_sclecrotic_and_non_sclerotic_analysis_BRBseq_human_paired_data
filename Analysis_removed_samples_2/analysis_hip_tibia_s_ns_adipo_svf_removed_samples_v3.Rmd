...
```{r}
# Prepare annotation data, select the top 2000 HVG and use the features, for all the conditions which include, tibia hip and pellet and adipo samples.
...
# Get most variable genes (top 2k)
#gene_vars <- apply(tibia_pellet_sclerotic_vs_non_sclerotic$normalized_matrix, 1, var, na.rm = TRUE)
gene_vars <- apply(results$logcpm, 1, var, na.rm = TRUE)
top_var_genes <- names(sort(gene_vars, decreasing = TRUE)[1:2000])

norm_counts <- results$logcpm[top_var_genes,]
norm_counts <- na.omit(norm_counts)
