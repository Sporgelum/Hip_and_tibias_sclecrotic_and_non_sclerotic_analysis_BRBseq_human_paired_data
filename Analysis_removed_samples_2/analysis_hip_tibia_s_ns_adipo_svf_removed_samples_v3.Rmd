...
# Patient-wise splitting
samples_per_patient <- table(md_df_bone$sample_number[md_df_bone$sampleID %in% combined_data$sample_2])
#paired_patients <- names(samples_per_patient)[samples_per_patient == 4 | samples_per_patient == 2] # leave those that have 3 as test!
#pull all patients into one list 
all_patients <- unique(md_df_bone$sample_number[md_df_bone$sampleID %in% combined_data$sample_2])


set.seed(123)
n_train <- round(length(all_patients) * 0.8)
train_patients <- sample(all_patients, n_train)
test_patients  <- setdiff(all_patients, train_patients)


# Create train/test datasets using the patient id so there is no patient leakage between training and testing.
all.equal(sort(unique(gsub(x = md_df_bone$sampleID[md_df_bone$sample_number %in% train_patients],pattern = "_.*",replacement = ""))),sort(train_patients))
...
# Visu
train_data_clean[,c(2002:ncol(train_data_clean))]
test_data_clean[,c(2002:ncol(test_data_clean))]



# # 1. Drop IDs
train_data_clean$patientID <- factor(gsub(x = train_data_clean$sample_2,pattern = "_.*",replacement = ""))
test_data_clean$patientID <- factor(gsub(x = test_data_clean$sample_2,pattern = "_.*",replacement = ""))
#groupids <- train_data_clean$patientID


# drop_ids <- c("sample_2","sampleID","patientID","tissue","type")
# train_data_clean <- train_data_clean[, setdiff(names(train_data_clean), drop_ids)]
# test_data_clean  <- test_data_clean[, setdiff(names(test_data_clean), drop_ids)]


# # 2. Optionally drop highly correlated predictors
# library(caret)
# corr_mtx <- cor(train_data_clean[ , sapply(train_data_clean,is.numeric)])
# highCorr <- caret::findCorrelation(corr_mtx, cutoff = 0.95)  
# # columns with pairwise |r|>0.90
# if(length(highCorr)>0) {
#   toDrop <- colnames(corr_mtx)[highCorr]
#   message("removed if genes are correlated at more than 0.95")
#   train_data_clean <- train_data_clean[ , setdiff(names(train_data_clean), toDrop)]
#   test_data_clean  <- test_data_clean[ , setdiff(names(test_data_clean), toDrop)]
# }
...
# RANDOM FOREST MODEL TRAINING
# ================================================================
# Train Random Forest model with more detailed parameters --> Now determine mtry and ntree with grid.search.


# 2A. Define a patient‐wise resampling strategy
ctrl <- caret::trainControl(
  method          = "cv",
  number          = 5,                    # 5‐fold CV
  #index           = caret::groupKFold(groupids, k = 5),                   # ensures no patient is in both train/test within a fold
  index           = caret::groupKFold(train_data_clean$patientID, k = 5),                   # ensures no patient is in both train/test within a fold
  classProbs      = TRUE,
  summaryFunction = twoClassSummary,      # optimize for AUC
  savePredictions = "final"
)

# 2B. Set up a tuning grid for mtry (and optionally nodesize)
p <- ncol(train_data_clean) - 1            # minus your response column
grid <- expand.grid(
  mtry = floor(sqrt(p)) * c(0.5,1,2),       # e.g. half‐sqrt, sqrt, twice‐sqrt
  splitrule = "gini",                      # for caret’s 'ranger' method alternative
...
rf_tuned <- caret::train(
  condition ~ . -sample_2 -patientID -tissue -type,
  data      = train_data_clean,
  method    = "ranger",                    # faster, supports min.node.size
  metric    = "ROC",
  trControl = ctrl,
  tuneGrid  = grid,
  num.trees = 1000                          # start with 1 000 trees
)
...
plot(rf_tuned$finalModel)  # error vs # trees
...
#                          )
# 
# print(rf_model)   -- > this is substituted by caret rf
