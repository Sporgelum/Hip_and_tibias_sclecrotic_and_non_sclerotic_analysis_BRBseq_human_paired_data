...
```{r,fig.width=16,fig.height=95, paged.print=TRUE}
...
```
# MDS shows two samples that do really mix with sclerotic and a few sclerotic veryclose to non.sclerotic according to RF classification =)

# maybe remove the oabs scoring (as it is the combination the other physiological features)
```{r}
table(train_data_clean$condition)
table(test_data_clean$condition)
...
saveRDS(rf_tuned, "~/CLUSTER/Environments/2024_07_05_Lea_Lausanne_Hip_Tibia_BRBseq/Analysis_removed_samples_2/trained_models_rf/all_features_on_tibia_samples_rf_model_20250820.rds")
...
# Use data predict any sample (without oarsi score... usually random forest wants to have the same columns..)
# Therefore impute the missing columns...
# Use logcpm counts of the sample to predict.
```{r paged.print=TRUE}
...
for (feature in features_to_fill) {
  ...
  #tibia_samples_without_features_data[[feature]] <- factor(round(mean_val))
  binary_val <- ifelse(mean_val >= 0.5, 1, 0)
  tibia_samples_without_features_data[[feature]] <- factor(binary_val, levels = c(0, 1))
}
...
for (feature in features_to_fill) {
  ...
  #tibia_samples_without_features_data[[feature]] <- factor(round(mean_val))
  binary_val <- ifelse(mean_val >= 0.5, 1, 0)
  hip_samples_without_features_data[[feature]] <- factor(binary_val, levels = c(0, 1))
}
#tibia_samples_without_features_data$oabs_scoring <- mean(as.numeric(train_data_clean$oarsi_scoring), na.rm = TRUE)  # Or any other strategy
#tibia_samples_without_features_data$oarsi_scoring <- factor(round(tibia_samples_without_features_data$oarsi_scoring,digits = 0))
