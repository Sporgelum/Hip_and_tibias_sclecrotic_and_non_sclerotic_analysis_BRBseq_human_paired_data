...
# quick visu sanity check to see selected genes separate.
library(pheatmap)
annotation_col <-  md_df_tibia_extended |>  dplyr::select(Group,sampleID) |> tibble::column_to_rownames("sampleID")
...
combined_data_tibias$celltype <- factor(combined_data_tibias$tissue_celltype_lowercase)
...
drop_cols <- c("sampleID", "health_status", "tissue_celltype_lowercase", "celltype", "patientID")
train_data_clean <- train_data_clean[, !(names(train_data_clean) %in% drop_cols)]
test_data_clean  <- test_data_clean[, !(names(test_data_clean) %in% drop_cols)]

train_data_clean[,(tail(colnames(train_data_clean),15))]
test_data_clean[,(tail(colnames(test_data_clean),15))]
non_numeric_cols = c("condition","Gender","oarsi_scoring","oabs_scoring","cysts","fibrosis","blood_vessels", "trabeculae_thickened", "inflammation")
...
# Add metadata (BMI, Age, Gender)
heatmap_annotation <- annotation_col |> 
  dplyr::left_join(md_df_tibia_extended[,c("sampleID","Age","Gender","BMI","oarsi_scoring","oabs_scoring","cysts","fibrosis","blood_vessels", "trabeculae_thickened", "inflammation")], by = c("sample_2"="sampleID")) |>
  dplyr::select(-ID, -tissue, -type, -sample_2)
...
# DONE HIPs   
#=======================================
## ADD adipocyte and pellet information on the models in tibias.
#=======================================
