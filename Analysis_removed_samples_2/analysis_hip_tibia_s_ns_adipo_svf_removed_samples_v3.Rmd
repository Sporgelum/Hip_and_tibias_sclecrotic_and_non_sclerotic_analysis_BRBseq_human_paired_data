...
```{r}
# ================================================================
# PRE-PROCESSING & FEATURE SELECTION (on training data ONLY)
# ================================================================
...
# --- Ensure 'condition' is a factor with consistent levels ---
# This is a critical step to prevent prediction errors later
train_data_clean$condition <- factor(train_data_clean$condition)
test_data_clean$condition  <- factor(test_data_clean$condition, levels = levels(train_data_clean$condition))

# --- Define the grouping variable for CV ---
# This uses the original, unfiltered training data to ensure all samples are accounted for
groupids <- factor(gsub("_.*", "", train_data_clean$sample_2))




# --- Safely identify predictor columns ---
# We separate metadata/outcome from the gene predictors
outcome_name <- "condition"
all_cols <- names(train_data_clean)
# Assuming metadata columns might be present, we explicitly name them to exclude them from feature selection
# Adjust this list if you have other metadata columns
#metadata_cols <- c("sample_2", "Age", "Gender", "BMI", "type", "tissue", "patientID")
metadata_cols <- c("sample_2", "type", "tissue", "patientID","ID")
# Predictors are columns that are NOT the outcome and NOT metadata
predictor_names <- setdiff(all_cols, c(outcome_name, metadata_cols))


# --- Apply nearZeroVar ONLY to the numeric predictors ---
nzv <- nearZeroVar(train_data_clean[, predictor_names], saveMetrics = TRUE)
drop_nzv_cols <- rownames(nzv)[nzv$nzv]

if (length(drop_nzv_cols) > 0) {
  cat("Removing", length(drop_nzv_cols), "near-zero variance predictors.\n")
  # Update the list of predictors to use
  predictor_names <- setdiff(predictor_names, drop_nzv_cols)
} else {
  cat("No near-zero variance predictors found to remove.\n")
}


# --- Create final, clean data frames for modeling ---
# This ensures train/test sets have the exact same columns
final_train_data <- train_data_clean[, c(outcome_name, predictor_names)]
final_test_data  <- test_data_clean[,  c(outcome_name, predictor_names)]
...
# NOTE: If you still get an error that ROC metrics are missing, it's likely
# because a fold has samples from only one class. The best fix is to
# reduce 'k' (e.g., from 5 to 3), which makes each fold larger.
set.seed(123) # for reproducible folds
cv_folds <- groupKFold(groupids, k = 5) #️ TRY k = 3 IF THIS FAILS
...
ctrl <- trainControl(
  ...
  allowParallel   = TRUE # Recommended for speed
)

# 2B. Tuning grid
p_final <- ncol(final_train_data) - 1
grid <- expand.grid(
  mtry          = floor(sqrt(p_final) * c(0.5, 1, 2)),
  splitrule     = "gini",
...
rf_tuned <- train(
  ...
  tuneGrid  = grid,
  num.trees = 1000,
  importance = 'impurity' # Optional: to calculate variable importance (gini), need "accuracy" too use =permutation
)
...
# ================================================================
# Predictions
train_predictions <- predict(rf_tuned, final_train_data)
test_predictions  <- predict(rf_tuned, final_test_data)

# Confusion Matrices
train_cm <- caret::confusionMatrix(train_predictions, final_train_data$condition)
test_cm  <- caret::confusionMatrix(test_predictions,  final_test_data$condition)
