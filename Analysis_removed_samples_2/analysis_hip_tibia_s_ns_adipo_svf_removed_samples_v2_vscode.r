# this was run from vscode, so the path is different

# %%
setwd("/data/users/mbotos/Environments/2024_07_05_Lea_Lausanne_Hip_Tibia_BRBseq/")
...
gc()
.libPaths("/data/users/mbotos/R/RLibs4.3/")
...
## --- Read tables
# %%
matrix_counts_tibia <- read.table(file ="/data/users/mbotos/Environments/2024_07_05_Lea_Lausanne_Hip_Tibia_BRBseq/brbseq-nf-pipeline/results/AMP0211/count_matrix/L270524LF01_01.read.counts.sampleIDs.detailed.txt",sep = "\t",header = TRUE)
head(matrix_counts_tibia)
tail(matrix_counts_tibia)
dim(matrix_counts_tibia)
head(matrix_counts_tibia)
#%
matrix_counts_hip <- read.table(file ="/data/users/mbotos/Environments/2024_07_05_Lea_Lausanne_Hip_Tibia_BRBseq/brbseq-nf-pipeline/results/AMP0211/count_matrix/L270524LF02_01.read.counts.sampleIDs.detailed.txt",sep = "\t",header = TRUE)
...
# %%
md <- readxl::read_xlsx(path ="/data/users/mbotos/Environments/2024_07_05_Lea_Lausanne_Hip_Tibia_BRBseq/Analysis_removed_samples/metadata/sample_info_minimal_SIT.xlsx")
md$sampleID
#%%
...
# %%
agbmi_df <- readxl::read_xlsx(path ="/data/users/mbotos/Environments/2024_07_05_Lea_Lausanne_Hip_Tibia_BRBseq/Analysis_removed_samples/metadata/age-gender-BRBseq.xlsx")
agbmi_df
...
# %%
# Use if want to see it in the browser.
# library("httpgd")
# httpgd::hgd()

# %%
# PLOT the PCA
plot_the_pca(pca_df = m_pca_hip$matrix_pca,color_var = "tissue_celltype_lowercase",shape_var = "health_status",pc_x = "PC1",pc_y = "PC2",title = "PCA of RNA-seq Raw Data",subtitle = "Hips SVF and Adipocytes",text_var = "sample_number",percentVar_obj = m_pca_hip$percentVar_obj)
...
plot_the_pca(pca_df = m_pca_tibia$matrix_pca,color_var = "tissue_celltype_lowercase",shape_var = "health_status",pc_x = "PC1",pc_y = "PC2",title="PCA of RNA-seq Raw Data",subtitle = "Tibias SVF and Adipocytes",text_var = "sample_number",percentVar_obj = m_pca_tibia$percentVar_obj)
...
### Need to be run the run_pca_structuring before the plot the pca otherwise the objects mess up
# Generate the columns in the md_df
plot_the_pca(pca_df = m_pca_tibia_t$matrix_pca,color_var=NULL,shape_var=NULL,pc_x = "PC1",pc_y = "PC2",title="PCA of Transposed RNA-seq Raw Data", subtitle = "Tibias SVF and Adipocyte Transposed", text_var = "sample_number",percentVar_obj = m_pca_tibia_t$percentVar_obj)
...
### Need to be run the run_pca_structuring before the plot the pca otherwise the objects mess up
# Generate the columns in the md_df
plot_the_pca(pca_df = m_pca_hip_t$matrix_pca,color_var =  NULL,shape_var = NULL,pc_x = "PC1",pc_y = "PC2",title="PCA of Transposed RNA-seq Raw Data",subtitle = "Hips SVF and Adipocyte Transposed",text_var = "sample_number",percentVar_obj = m_pca_hip$percentVar_obj)
...
md_df_bone |>
  ...
  dplyr::pull(sampleID)
# colnames(md_df_tibia |> dplyr::filter(tissue_celltype_lowercase == "pellet"))
#enerate matrix for PCA of samples together.
# %%
...
volcano_tibia_pellet_sclerotic_vs_non_sclerotic <- plot_volcano(results_table = tibia_pellet_sclerotic_vs_non_sclerotic$results$table,
                                                                  ...
                                                                  gene_column = "genes",
                                                                  genes_of_interest = c("IL11","RCN2", "IL11", "CNTNAP2", "STMN2"))
#ibia sclerotic vs non sclerotic
...
#odel, try to predict condition (non.sclerotic vs sclerotic) using a model and the metadata.
...
importance(rf_model)
varImpPlot(rf_model)
#i: how goodd is the feature to splti the trees...
